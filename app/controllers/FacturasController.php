...
/*
Exportar excel desde grid facturas pendientes
 */
    public function exportexcelAction($n_rows, $columns, $filtros, $groups, $sorteds)
    {
        ...
        $generalConfigForAllColumns = array(
            ...
            'grupo' => array('title' => 'Grupo', 'width' => 20, 'align' => 'L', 'type' => 'varchar'),
            'linea' => array('title' => 'Linea', 'width' => 20, 'align' => 'L', 'type' => 'varchar'),
            'estacion' => array('title' => 'Estacion', 'width' => 15, 'align' => 'L', 'type' => 'varchar'),
            'razon_social' => array('title' => 'Razon Social', 'width' => 20, 'align' => 'L', 'type' => 'varchar'),
            ...
            'monto_reprogramado' => array('title' => 'Monto Programado', 'width' => 30, 'align' => 'R', 'type' => 'numeric'),
            'diferencia_dias' => array('title' => 'Dias Atraso', 'width' => 30, 'align' => 'R', 'type' => 'int4'),
        );
        ...
        if (count($widthsSelecteds) > 0) {
            ...
            //$excel->secondPage();
            if ($excel->debug == 0) {
                $excel->display("AppData/reporte_facturas_pendientes.xls", "I");
            }
...
/*
exportar excel Grid Facutras realizadas
 */

public function exportexcelfacturasrealizadasAction($n_rows, $columns, $filtros, $groups, $sorteds)
    {
        $this->view->disable();
        $columns = base64_decode(str_pad(strtr($columns, '-_', '+/'), strlen($columns) % 4, '=', STR_PAD_RIGHT));
        $filtros = base64_decode(str_pad(strtr($filtros, '-_', '+/'), strlen($columns) % 4, '=', STR_PAD_RIGHT));
        ...
        $generalConfigForAllColumns = array(
            ...
            'fecha_programado' => array('title' => 'Fecha Programado', 'width' => 8, 'align' => 'C', 'type' => 'date'),
            'monto_reprogramado' => array('title' => 'Monto Programado', 'width' => 30, 'align' => 'R', 'type' => 'numeric'),
            'nro_factura' => array('title' => 'Nro Factura', 'width' => 30, 'align' => 'R', 'type' => 'int4'),
            'fecha_factura' => array('title' => 'Fecha Factura', 'width' => 30, 'align' => 'C', 'type' => 'date'),
            'monto_factura' => array('title' => 'Monto Facturado', 'width' => 30, 'align' => 'R', 'type' => 'int4'),
        );
        $agruparPor = ($groups != "") ? explode(",", $groups) : array();
        $widthsSelecteds = $this->DefineWidths($generalConfigForAllColumns, $columns, $agruparPor);
        $ancho = 0;
        if (count($widthsSelecteds) > 0) {
            ...
            #region Proceso de generación del documento Excel
            $excel->debug = 0;
            $excel->title_rpt = utf8_decode('Reporte Facturas Realizadas');
            $excel->header_title_empresa_rpt = utf8_decode('Empresa Estatal de Transporte por Cable "Mi Teleférico"');
            $alignSelecteds = $excel->DefineAligns($generalConfigForAllColumns, $columns, $agruparPor);
            $colSelecteds = $excel->DefineCols($generalConfigForAllColumns, $columns, $agruparPor);
            $colTitleSelecteds = $excel->DefineTitleCols($generalConfigForAllColumns, $columns, $agruparPor);
            ...
            if ($excel->debug == 1) echo "<p>WHERE------------------------->" . $where . "<p>";
            if ($excel->debug == 1) echo "<p>GROUP BY------------------------->" . $groups . "<p>";
            $resul = $obj->listafacturas($where,$groups);
            ...
            foreach ($resul as $v) {
                ...
                $relaboral[] = array(
                    ...
                    'monto_reprogramado' => number_format($v->monto_reprogramado,2,',',''),
                    'nro_factura' => $v->nro_factura,
                    'fecha_factura' => date("d-m-Y",strtotime($v->fecha_factura)),
                    'monto_factura' => number_format($v->monto_factura,2,',',''),
                );
            }
            #region Espacio para la definición de valores para la cabecera de la tabla
            $excel->FechaHoraReporte = date("d-m-Y H:i:s");
            ...
            //$excel->secondPage();
            if ($excel->debug == 0) {
                $excel->display("AppData/reporte_facturas_realizadas.xls", "I");
            }
            #endregion Proceso de generación del documento PDF
        }
    }
